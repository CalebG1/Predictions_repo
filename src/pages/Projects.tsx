...
const seedProjects: Project[] = [
  ...
  },
  {
    ...
    shortName: "HR",
    description:
      "Deliver a 186-unit mixed-use waterfront building with ground-floor retail and public access.",
  ...
  },
  {
    ...
    shortName: "TC",
    description:
      "Install a 50,000-ton-per-year capture unit on an operating cement kiln without disrupting output.",
  ...
  },
  {
    ...
    shortName: "IO",
    description:
      "Develop and analytically validate a blood-based recurrence test for stage II colorectal cancer.",
  ...
  },
  {
    ...
    shortName: "FL",
    description:
      "Retrofit a live precision-parts line with robotic inspection and closed-loop process control.",
  ...
  },
  {
    ...
    shortName: "SC",
    description:
      "Open six hybrid-care clinics and a telehealth hub serving three medically underserved counties.",
    industry: "Healthcare",
    icon: "medical",
    color: "#cf4e5c",
...
function ProjectIcon({ name }: { name: string }) {
  const paths: Record<string, JSX.Element> = {
    car: (
      <>
        <path d="M5 16h14l-1.3-5.2A2.4 2.4 0 0 0 15.4 9H8.6a2.4 2.4 0 0 0-2.3 1.8L5 16Z" />
    ...
    dna: (
      <>
        <path d="M7 3c0 7 10 11 10 18M17 3C17 10 7 14 7 21M8 6h8M7 11h10M7 16h10M8 20h8" />
      </>
    ),
    satellite: (
      <>
        <path d="m9 9 6 6M7 13l4 4 6-6-4-4-6 6ZM5 5l4 4M15 15l4 4M2 8l6-6 4 4-6 6-4-4ZM12 18l6-6 4 4-6 6-4-4Z" />
      </>
    ),
    factory: (
      <>
        <path d="M3 21V9l6 3V9l6 3V5h4v16H3Z" />
...
function Sparkline({ values, health }: { values: number[]; health: ProjectHealth }) {
  ...
  return (
    <svg
      className={`pm-spark pm-spark-${health.toLowerCase().replace(" ", "-")}`}
    ...
    <article className="pm-project-card">
      <div className="pm-card-head">
        ...
        <div className="pm-card-title">
          ...
          <button className="pm-project-title-link" onClick={onOpen}>
            <h3>{project.name}</h3>
          </button>
        </div>
        <div className="pm-card-actions">
          <span className={`pm-health pm-health-${project.health.toLowerCase().replace(" ", "-")}`}>
            {project.health}
          </span>
          <button
            className={`pm-watch${project.watched ? " active" : ""}`}
            onClick={onToggleWatch}
            title={project.watched ? "Remove from watchlist" : "Add to watchlist"}
            aria-label={project.watched ? "Remove from watchlist" : "Add to watchlist"}
          >
      ...
      <div className="pm-health-row">
        <div>
          ...
          <div className="pm-probability">
            <strong>{project.deliveryConfidence}%</strong>
            <Trend change={project.change} trend={project.trend} />
          </div>
        </div>
      ...
      <div className="pm-key-forecast">
        <div className="pm-key-forecast-top">
          <span>Key forecast</span>
          <strong>{project.topForecastProbability}%</strong>
        </div>
      ...
      <div className="pm-risk-row">
        ...
        {project.risks.length ? (
          project.risks.slice(0, 2).map((risk) => (
            <span className="pm-risk-chip" key={risk}>
              {risk}
            </span>
          ))
        ) : (
          <span className="pm-no-risk">No active blockers</span>
        )}
        {project.risks.length > 2 && (
          <span className="pm-risk-more">+{project.risks.length - 2}</span>
        )}
      </div>

      <div className="pm-card-foot">
        <div className="pm-owner">
          <span className="pm-avatar" style={{ background: project.color }}>
        ...
        <div className="pm-forecast-meta">
          <strong>{project.forecastCount}</strong> questions ·{" "}
          <strong>{project.forecasters}</strong> forecasters
        </div>
      </div>
    </article>
  );
...
function ProjectListRow({
  ...
  return (
    <article className="pm-list-row">
      ...
      <div
        className="pm-project-mark pm-project-mark-small"
        style={{ background: `${project.color}14`, color: project.color }}
      >
        <ProjectIcon name={project.icon} />
      </div>
      <div className="pm-list-name">
      ...
      <div className="pm-list-owner">
        <span className="pm-avatar" style={{ background: project.color }}>
          {project.ownerInitials}
        </span>
        <span>{project.owner}</span>
      </div>
      <div className="pm-list-progress">
        <span>{project.progress}% complete</span>
        <div className="pm-progress">
          <span style={{ width: `${project.progress}%`, background: project.color }} />
        </div>
      </div>
      <div className="pm-list-date">
        <span>{project.nextMilestone}</span>
        <strong>{project.targetDate}</strong>
      </div>
      <div className="pm-list-prob">
        <strong>{project.deliveryConfidence}%</strong>
        <Trend change={project.change} trend={project.trend} />
      </div>
      <span className={`pm-health pm-health-${project.health.toLowerCase().replace(" ", "-")}`}>
...
export default function Overview() {
  ...
  const filteredProjects = useMemo(() => {
    ...
    const result = projects.filter((project) => {
      const matchesQuery =
        !query ||
        [project.name, project.description, project.industry, project.owner, project.location].some(
          (value) => value.toLowerCase().includes(query),
        );
      return (
        matchesQuery &&
        (health === "All" || project.health === health) &&
        (industry === "All industries" || project.industry === industry)
      );
    });
  ...
  const portfolioConfidence = Math.round(
    projects.reduce((total, project) => total + project.deliveryConfidence, 0) / projects.length,
  );
  ...
  const toggleWatch = (id: string) =>
    setProjects((current) =>
      current.map((project) =>
        project.id === id ? { ...project, watched: !project.watched } : project,
      ),
    );
  ...
  const addProject = (event: FormEvent<HTMLFormElement>) => {
    ...
    const newProject: Project = {
      ...
      shortName: name
        ...
        .toUpperCase(),
      description: String(
        data.get("description") || "New project ready for scope and forecast questions.",
      ),
      ...
      ownerInitials: String(data.get("owner") || "UA")
        .split(" ")
        .map((word) => word[0])
        .slice(0, 2)
        .join("")
      ...
      <header className="pm-page-head">
        <div>
          <div className="pm-kicker">
            <span className="pm-live-dot" /> Portfolio intelligence · Updated 8 minutes ago
          </div>
          <h1>Projects</h1>
          <p>
        ...
        <button className="pm-primary-btn" onClick={() => setCreateOpen(true)}>
      ...
      <section className="pm-stats" aria-label="Portfolio summary">
        <div className="pm-stat pm-stat-primary">
          ...
          <div>
            ...
            <strong>{portfolioConfidence}%</strong>
            <small>+2 pts this month</small>
          </div>
        </div>
        <div className="pm-stat">
          <div className="pm-stat-icon pm-stat-icon-red">
            <svg viewBox="0 0 24 24">
          ...
          <div>
            ...
            <small>2 changed this week</small>
          </div>
        </div>
        <div className="pm-stat">
          ...
          <div>
            ...
            <strong>{totalForecasts}</strong>
            <small>37 updated today</small>
          </div>
        </div>
        <div className="pm-stat">
          ...
          <div>
            ...
            <small>Across 3 industries</small>
          </div>
        </div>
      </section>

      <section className="pm-briefing">
        <div className="pm-briefing-badge">
          <svg viewBox="0 0 24 24">
            <path d="M12 3v3M5.6 5.6l2.1 2.1M3 12h3M18 12h3M16.3 7.7l2.1-2.1" />
            <path d="M9 18h6M10 21h4" />
            <path d="M8 14a5 5 0 1 1 8 0c-1 1-1 2-1 2H9s0-1-1-2Z" />
          </svg>
        </div>
        <div className="pm-briefing-copy">
          <span>Portfolio briefing</span>
          <strong>Attention shifted to Harborline Residences</strong>
          <p>
            On-time delivery fell 12 points after the facade supplier moved its committed ship date.
            Forecasters see a 38% chance the package arrives before crane demobilization.
          </p>
        </div>
        <button
          onClick={() => {
        ...
        >
          Review 3 at-risk projects <span>→</span>
        </button>
      </section>
      ...
      <section className="pm-portfolio-section">
        <div className="pm-section-title">
          ...
          <div className="pm-health-tabs" aria-label="Filter by health">
            {(["All", "On track", "Watch", "At risk"] as const).map((item) => (
              <button
                key={item}
              ...
              >
                {item}
                {item !== "All" && (
                  <span>{projects.filter((project) => project.health === item).length}</span>
                )}
              </button>
            ))}
          </div>
        ...
        <div className="pm-toolbar">
          <label className="pm-search">
            ...
            <input
              ...
              placeholder="Search projects, owners, or locations…"
            />
          </label>
          <select
            value={industry}
          ...
          <label className="pm-sort-label">
            <span>Sort</span>
            <select value={sort} onChange={(event) => setSort(event.target.value)}>
              <option>Needs attention</option>
              <option>Highest confidence</option>
              <option>Nearest milestone</option>
        ...
        {filteredProjects.length ? (
            ...
            <div className="pm-project-grid">
              {filteredProjects.map((project) => (
                <ProjectCard
                  key={project.id}
                  project={project}
                  onToggleWatch={() => toggleWatch(project.id)}
          ...
          <div className="pm-empty">
            ...
            <p>Try a different search or clear one of your filters.</p>
            <button
              onClick={() => {
                setSearch("");
                setHealth("All");
      ...
      {createOpen && (
        ...
        >
          ...
          >
            ...
            <p className="pm-modal-intro">
              Add the essentials now. Forecast questions, milestones, and evidence sources can be
              configured next.
            </p>
            <form onSubmit={addProject}>
              <label>
                ...
                <input
                  name="name"
                  required
                  placeholder="e.g. Delta water treatment expansion"
                  autoFocus
                />
              </label>
              <label>
                <span>Short description</span>
                <textarea name="description" placeholder="What are you delivering, and for whom?" />
              </label>
              <div className="pm-form-grid">
                <label>
                  ...
                  <select name="industry">
                    ...
                    <option>Climate infrastructure</option>
                  </select>
                </label>
                <label>
                  <span>Project lead</span>
                  <input name="owner" required placeholder="Full name" />
                </label>
                <label>
                  <span>Location</span>
                ...
                <label>
                  ...
                  <input name="date" placeholder="e.g. Dec 15, 2026" />
                </label>
              </div>
              <div className="pm-modal-actions">
                ...
                <button className="pm-primary-btn" type="submit">
                  Create project
                </button>
              </div>
            </form>
